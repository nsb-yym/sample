...
public class CopyOfCopyOfCopyOfCopyOfasdfsadf1123BaseAutomationLogic extends BaseAutomationLogic {
	...
	public void webservice(ParameterSet globalParameterSet) throws AutomationFailException {//!findOffset_webservice!
		ParameterSet tmpParameterSet = new ParameterSet();

		HashMap  provisionalNameMap = new HashMap();

		ResultNameSet resultNameSet = new ResultNameSet();

		WSInvokingInfo info = new WSInvokingInfo();
		info.setCharset("UTF-8");
		info.setDomainName("aa");
		info.setDataSourceName("");
		info.setTargetClassName("null");
		info.setPortName("null");
		info.setOperationClassname("null");
		info.setOperationName("null");
		info.setPostfix("null");
		info.setProvisionalNameMap(provisionalNameMap);
		info.setResultNameSet(resultNameSet);
		info.setParameterSet(tmpParameterSet);
		// WSDataSourceEvent
		WSDataSourceEvent dataSourceEvent = new WSDataSourceEvent() {
			public void onBeforeExecute(ParameterSet globalParameterSet, InvokingInfo invokingInfo, DataSource dataSource) throws AutomationFailException {//!findOffset_webserviceonBeforeExecute!
				webserviceonBeforeExecute(globalParameterSet, invokingInfo, dataSource);
			}
			public void onAfterExecute(ParameterSet globalParameterSet, InvokingInfo invokingInfo, DataSource dataSource) throws AutomationFailException {//!findOffset_webserviceonAfterExecute!
				webserviceonAfterExecute(globalParameterSet, invokingInfo, dataSource);
			}
		};
		WSFlowEvent event = new WSFlowEvent(){
			public void onBegin(ParameterSet globalParameterSet, ParameterSet inputParameterSet, InvokingInfo invokingInfo) throws AutomationFailException {//!findOffset_webserviceonBegin!
			
			}
			public void onEnd(ParameterSet globalParameterSet, ParameterSet inputParameterSet, InvokingInfo invokingInfo) throws AutomationFailException {//!findOffset_webserviceonEnd!
			
			}
			public void onExceptionOccured(ParameterSet globalParameterSet, InvokingInfo invokingInfo, Throwable e, InvokingErrorInfo errorInfo) throws AutomationFailException {//!findOffset_webserviceonExceptionOccured!
				webserviceonExceptionOccured(globalParameterSet, invokingInfo, e, errorInfo);
			}
		};
		EventHandler eventHandler = new EventHandler();
		eventHandler.addEvent(event);
	...
	public void webserviceonAfterExecute(ParameterSet globalParameterSet, InvokingInfo invokingInfo, DataSource dataSource) throws AutomationFailException {
		
	}
	public void webserviceonBeforeExecute(ParameterSet globalParameterSet, InvokingInfo invokingInfo, DataSource dataSource) throws AutomationFailException {
		
	}
	public void webserviceonExceptionOccured(ParameterSet globalParameterSet, InvokingInfo invokingInfo, Throwable e, InvokingErrorInfo errorInfo) throws AutomationFailException {
		throw new AutomationFailException(e.getMessage(), e);
	}
